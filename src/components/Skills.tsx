...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-white to-purple-50/50 dark:from-gray-900 dark:to-gray-800/50 relative overflow-hidden">
      {/* Background decorative elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-32 right-32 w-80 h-80 bg-gradient-to-br from-purple-400/10 to-pink-400/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-32 left-32 w-80 h-80 bg-gradient-to-tr from-blue-400/10 to-purple-400/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {Object.entries(t.skills.categories).map(([key, category]) => (
            <div key={key} className="bg-white/30 dark:bg-gray-800/30 backdrop-blur-xl rounded-2xl p-4 sm:p-6 border border-white/20 dark:border-gray-700/20 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, skillIndex) => (
                  <span
                    key={skillIndex}
                    className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm border hover:scale-105 transition-all duration-200 ${getSkillColor(skill)}`}
                  >
                    {skill}
                  </span>
                ))}
