...
const HelloIntero: React.FC = () => {
  const helloRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Refresh animation every 20 seconds
    const interval = setInterval(() => {
      if (helloRef.current) {
        helloRef.current.style.display = 'none';
        setTimeout(() => {
          if (helloRef.current) {
            helloRef.current.style.display = 'flex';
          }
        }, 10);
      }
    }, 20000);
  ...
  return (
    <div className="hello-intero-bg fixed inset-0 w-full h-full">
      <div ref={helloRef} className="hello-intero-container">
        <svg className="hello-intero-svg" viewBox="-50 -50 1330.94 514.57">
          <path
